...
publish_event() {
  ...
  local event_id event_file response_timestamp event_json
  ...
  event_json=$(cat <<EOF
{"event_id":"$event_id","event_type":"$event_type","emitted_at_utc":"$(utc_now)","service":"$SERVICE_NAME","trace_id":"$trace_id_value","request":{"method":"$method","path":"$path","trace_id":"$trace_id_value"},"data":{"route":"$path","response_message":"$response_message","response_timestamp":$response_timestamp}}
EOF
)
...
EOF
      ...
      body=$(cat <<EOF
{"status":"success","trace_id":"$trace_id_value","message":"Current server time","event_status":"$event_status","data":{"timestamp":$timestamp_value,"datetime_utc":"$utc_value","datetime_th":"$th_value","timezone":"Asia/Bangkok","utc_offset":"+07:00"},"request":{"method":"$method","path":"$path","trace_id":"$trace_id_value"}}
EOF
)
      json_response "$body"
      ...
      body=$(cat <<EOF
{"status":"success","trace_id":"$trace_id_value","message":"Service is healthy","event_status":"$event_status","data":{"service":"$SERVICE_NAME","healthy":true,"event_consumer":"background file queue processor","port":$PORT},"request":{"method":"$method","path":"$path","trace_id":"$trace_id_value"}}
EOF
)
      json_response "$body"
    else
      ...
      body=$(cat <<EOF
{"status":"error","trace_id":"$trace_id_value","message":"Route not found","event_status":"$event_status","data":null,"request":{"method":"$method","path":"$path","trace_id":"$trace_id_value"}}
EOF
)
